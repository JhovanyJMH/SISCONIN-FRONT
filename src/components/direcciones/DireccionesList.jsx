...
import { fetchDirecciones, deleteDireccion, clearError, clearSuccessMessage, clearHighlightedDireccion, toggleStatusDireccion } from '../../features/direcciones/direccionesSlice';
...
const DireccionesList = () => {
  ...
  const { 
    ...
    successMessage, 
    pagination,
    highlightedDireccion,
    paginationInfo
  } = useSelector((state) => state.direcciones);
  ...
  useEffect(() => {
    ...
    if (paginationInfo && !hasNavigated) {
      ...
      dispatch(fetchDirecciones({ page: paginationInfo.page, search: '', status: 'activo' }));
    } else {
      // Para navegación manual o carga inicial
      dispatch(fetchDirecciones({ page: currentPage, search: searchTerm, status: 'activo' }));
    }
  }, [dispatch, currentPage, searchTerm, paginationInfo, hasNavigated]);

  // Efecto para la animación de parpadeo y scroll
  useEffect(() => {
    if (highlightedDireccion) {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    const result = await Swal.fire({
      title: '¿Estás seguro?',
      text: `¿Deseas ${currentStatus ? 'desactivar' : 'activar'} esta dirección?`,
      icon: 'question',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
    ...
    if (result.isConfirmed) {
      dispatch(toggleStatusDireccion(id));
    }
  };
  ...
  return (
    <>
      <SearchBar 
        searchPath="/get-search-direcciones"
        normalPath="/get-direcciones"
        showButton={false}
        ...
        <table className="w-full text-sm text-left text-gray-700">
          <thead className="text-base text-white uppercase bg-colorPrimario">
            <tr>
              ...
              <th scope="col" className="py-2 px-4">Dirección</th>
             {/* <th scope="col" className="py-2 px-4">Estado</th>
                 <th scope="col" className="py-2 px-4">Acciones</th>
                 */}
            </tr>
          </thead>
          <tbody className={`${loading ? 'hidden' : ''} uppercase text-xs`}>
            {direcciones.map((direccion) => (
              <tr
                key={direccion.id}
                ref={highlightedDireccion === direccion.id ? highlightedRowRef : null}
                className={`border-b cursor-pointer transition-all duration-300 ${
                  highlightedDireccion === direccion.id 
                    ? isHighlighted 
              ...
              >
                ...
                <td className="py-1 px-4 max-w-xs truncate" title={direccion.direccion}>
                  {direccion.direccion}
                </td>
                {/*
                ...
                <td className="py-1 px-4">
                  <button
                    onClick={() => handleToggleStatus(direccion.id, direccion.activo)}
                    title={direccion.activo ? 'Desactivar' : 'Activar'}
                  ...
                  >
                    {direccion.activo ? 'Desactivar' : 'Activar'}
                  </button>
                </td>
                */}
              </tr>
