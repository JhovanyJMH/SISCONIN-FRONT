...
import { getHistorial } from '../../features/inventarioRentado/inventarioRentadoSlice';
...
const HistorialRentadoList = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const location = useLocation();
  const { equipoId } = useParams();
  const [isLoading, setIsLoading] = useState(true);
  const { historial, pagination } = useSelector(state => state.inventarioRentado);
  ...
  const handlePageChange = (pageNumber) => {
    ...
    navigate(`/inventarios/rentado/historial/${equipoId}?page=${pageNumber}`);
  };

  const handleVerPDF = async (inventarioId) => {
    try {
      const pdfResponse = await fetch(`${BASE_URL}/api/equipo/${equipoId}/ficha-pdf-historial/${inventarioId}`, {
        method: 'GET',
        headers: {
  ...
  const handleVerDetalle = (registro) => {
    navigate(`/inventarios/rentado/historial/${equipoId}/detalle/${registro.id}`, { state: { registro } });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => navigate(`/inventarios/rentado/asignacion/${equipoId}`)}
...
export default HistorialRentadoList; 