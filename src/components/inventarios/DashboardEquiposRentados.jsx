...
const DashboardEquiposRentados = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const dashboardData = await equiposRentadosService.getDashboardData();
  ...
  const handleDownloadPDF = async () => {
    ...
    const fileName = `Resumen de Equipos Rentados ${formattedDate}.pdf`;
    ...
    const title = 'Resumen de Equipos Rentados';
    ...
    autoTable(doc, {
      ...
      head: [['Total Equipos',  'Sin Asignación']],
      body: [
    ...
    // Por Perfil
    doc.text('Equipos por Perfil:', 14, y);
    autoTable(doc, {
      startY: y + 2,
      head: [['Perfil', 'Total']],
      body: data.por_perfil.map(item => [item.equipo || 'Sin perfil', item.total]),
      theme: 'grid',
      styles: { fontSize: 7 },
      headStyles: { fontSize: 7, fillColor: [138, 32, 54] },
    });
    ...
    // Por Modelo
 // doc.text('Equipos por Modelo:', 14, y);
 // autoTable(doc, {
 //   startY: y + 2,
 //   head: [['Modelo', 'Total']],
 //   body: data.por_modelo.map(item => [item.eq_modelo || 'Sin modelo', item.total]),
 //   theme: 'grid',
 //   styles: { fontSize: 7 },
 //   headStyles: { fontSize: 7, fillColor: [138, 32, 54] },
 // });
 // y = doc.lastAutoTable.finalY + 6;
    ...
    // Por Dependencia
    doc.text('Asignaciones Unidad Administrativa:', 14, y);
    autoTable(doc, {
      startY: y + 2,
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 mb-8">
      <h2 className="text-xl font-bold mb-4 text-colorPrimario text-center">Resumen de Equipos Rentados</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-green-100 rounded p-4 text-center">
          <div className="text-3xl font-bold">{data.total_equipos}</div>
          <div className="text-sm">Total Equipos</div>
        </div>
       {/* <div className="bg-green-100 rounded p-4 text-center">
          <div className="text-3xl font-bold">{data.total_rentados}</div>
          <div className="text-sm">Rentados</div>
        </div>
        <div className="bg-yellow-100 rounded p-4 text-center">
          <div className="text-3xl font-bold">{data.total_no_rentados}</div>
          <div className="text-sm">No Rentados</div>
        </div>
        */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div>
          <h3 className="font-semibold mb-2">Por perfil del equipo</h3>
          <div className="max-h-[420px] overflow-y-auto scrollbar-hide">
            <ul className="text-sm">
              {data.por_perfil.map((item, idx) => (
                <li key={idx} className="flex justify-between border-b py-1">
                  <span>{item.equipo || 'Sin perfil'}</span>
                  <span className="font-bold">{item.total}</span>
                </li>
              ))}
            </ul>
        ...
        {/*
          <div>
          <h3 className="font-semibold mb-2">Por Modelo</h3>
          <div className="max-h-[420px] overflow-y-auto scrollbar-hide">
            <ul className="text-sm">
              {data.por_modelo.map((item, idx) => (
                <li key={idx} className="flex justify-between border-b py-1">
                  <span>{item.eq_modelo || 'Sin modelo'}</span>
                  <span className="font-bold">{item.total}</span>
                </li>
              ))}
            </ul>
   ...
   {/*   {data.por_usuario && data.por_usuario.length > 0 && (
        <div className="mt-8">
          <h3 className="font-semibold mb-2">Por Usuario (Asignaciones Activas)</h3>
...
export default DashboardEquiposRentados; 